...
def emap_potential(cubic,mu,cubic_mu,alpha,value='mu'):
    if cubic:
        replica_num=int(alpha*(cubic_mu.shape[0]-1))
        if value == 'mu':
            mu=np.array([np.zeros((2))]*cubic_mu.shape[0])
            potential=mu[replica_num].astype(np.float64)
        elif value == 'cubic_mu':
            potential=cubic_mu[replica_num].astype(np.float64)
    else:
        replica_num=int(alpha*(mu.shape[0]-1))
        if value == 'mu':
            potential=mu[replica_num].astype(np.float64)
        elif value == 'cubic_mu':
            cubic_mu=np.array([np.zeros((2))]*mu.shape[0])
            potential=cubic_mu[replica_num].astype(np.float64)
    return potential
...
def _add_meld_restraint(rest, meld_force, alpha, timestep):
    ...
    if isinstance(rest, restraints.DistanceRestraint):
        ...
        rest.cubic,
        emap_potential(rest.cubic,rest.mu,rest.cubic_mu,alpha,value='mu'),
        emap_potential(rest.cubic,rest.mu,rest.cubic_mu,alpha,value='cubic_mu'),
