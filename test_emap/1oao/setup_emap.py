...
#    n_res = len(sequence.split())
    templates = glob.glob('./1OAO_s_centered.pdb')
...
#    
    ...
    psi=np.round(md.compute_psi(md.load_pdb('1OAO_s_centered.pdb'))[1][0]*180/np.pi,2)
    for i in range(1,n_res):
        if i in HB:
            ...
            psi_rest = s.restraints.create_restraint('torsion', dist_scaler,
                                                      phi=psi_avg, delta_phi=psi_sd, k=1,
    ...
    phi=np.round(md.compute_phi(md.load_pdb('1OAO_s_centered.pdb'))[1][0]*180/np.pi,2)
    for i in range(2,n_res+1):            
        if i in HB:
            ...
            phi_rest = s.restraints.create_restraint('torsion', dist_scaler,
                                             phi=phi_avg, delta_phi=phi_sd, k=1,
